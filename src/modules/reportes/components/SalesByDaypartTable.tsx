...
interface Props {
  data: VentaPorDaypart[];
}

const DAYPART_STYLE: Record<string, { bg: string; text: string }> = {
  Desayuno: { bg: 'bg-amber-100', text: 'text-amber-700' },
  Almuerzo: { bg: 'bg-orange-100', text: 'text-orange-700' },
  Cena:     { bg: 'bg-red-100',   text: 'text-red-700'   },
};

export default function SalesByDaypartTable({ data }: Props) {
  if (data.length === 0) {
    return (
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-gray-100 bg-gray-50">
              {['Fecha', 'Daypart', 'Restaurante', 'Pedidos', 'Total Ventas'].map((h) => (
          ...
          <tbody>
            {data.map((row, i) => {
              const s = DAYPART_STYLE[row.Daypart] ?? DAYPART_STYLE.Almuerzo;
              return (
